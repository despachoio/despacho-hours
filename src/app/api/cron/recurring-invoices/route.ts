...
export async function GET(request: Request) {
  ...
  const summary = { processed: schedules?.length || 0, generated: 0, sent: 0, failed: 0, skipped: 0, errors: [] as { scheduleId: string; error: string }[] };
  for (const schedule of schedules || []) {
    try {
      ...
      summary.generated++;
      if (generated.autoSend) { await sendRecurringInvoice(admin, generated.invoice.id); summary.sent++; }
    } catch (scheduleError) {
