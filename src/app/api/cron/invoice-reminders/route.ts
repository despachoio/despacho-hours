...
function reminderContent(
  ...
  const text = renderSettingsTemplate(
    (isOverdue
...
export async function GET(request: Request) {
  ...
  const { data: invoices, error } = await admin
    .from("invoices")
    .select(
      "id,invoice_number,status,due_date,currency,total_amount,sent_to,sent_cc,reminders_enabled,reminder_count,next_reminder_at,clients(name)",
    )
  ...
  for (const invoice of invoices || []) {
    try {
      ...
      let { data: reminder, error: reserveError } = await admin
