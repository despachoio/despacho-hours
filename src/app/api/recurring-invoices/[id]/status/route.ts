...
export async function POST(
  ...
  if (!["active", "paused", "cancelled"].includes(status))
    return Response.json({ error: "Invalid schedule status" }, { status: 400 });
  const nextDate =
    typeof body.nextGenerationDate === "string" ? body.nextGenerationDate : "";
  const cancellationReason =
    typeof body.cancellationReason === "string"
      ? body.cancellationReason.trim()
      : "";
  if (status === "active" && !/^\d{4}-\d{2}-\d{2}$/.test(nextDate))
    return Response.json(
      { error: "Next generation date is required when resuming" },
      { status: 400 },
    );
  ...
  if (status === "active")
    await refreshPendingOccurrences(
      auth.admin,
      result.data as RecurringSchedule,
      auth.user.id,
    );
