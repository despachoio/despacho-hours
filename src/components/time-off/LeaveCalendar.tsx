...
export default function LeaveCalendar({ leave, holidays }: { leave: CalendarLeave[]; holidays: Holiday[] }) {
  ...
  const managers = Array.from(new Map(leave.filter((item) => item.reporting_manager_id).map((item) => [item.reporting_manager_id as string, { name: item.manager_name || "Manager", code: item.manager_code }])).entries()).sort(([, left], [, right]) => compareEmployeeCodes(left.code, right.code, left.name, right.name));
  const employees = Array.from(new Map(leave.map((item) => [item.employee_id, { name: item.employee_name, code: item.employee_code }])).entries()).sort(([, left], [, right]) => compareEmployeeCodes(left.code, right.code, left.name, right.name));
