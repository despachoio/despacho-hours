...
type Tab = "overview" | "requests" | "calendar" | "approvals" | "admin";
...
export default function TimeOffWorkspace() {
  ...
  return <main className="min-h-screen bg-[#f8fafc] px-5 py-7 sm:px-8">
    <div className="mx-auto max-w-[1500px]">
      <header className="relative overflow-hidden rounded-[2rem] bg-gradient-to-br from-[#081226] via-[#102656] to-[#153E90] px-7 py-8 text-white shadow-[0_28px_75px_-35px_rgba(15,23,42,.85)] sm:px-10 lg:py-10">
        ...
        <div className="relative flex flex-wrap items-center justify-between gap-6"><div className="flex items-start gap-4"><span className="mt-1 flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl border border-white/15 bg-white/10 shadow-xl backdrop-blur"><TimeOffIcon name="calendar" className="h-7 w-7 text-cyan-200" /></span><div><p className="text-xs font-bold uppercase tracking-[0.22em] text-cyan-200">People & wellbeing</p><h1 className="mt-2 text-4xl font-bold tracking-tight lg:text-5xl">Time Off</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-slate-300">Plan leave, protect team capacity, and keep every policy decision clear and auditable.</p><span className="mt-4 inline-flex rounded-full border border-white/10 bg-white/10 px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-blue-100 backdrop-blur">{role || "employee"} experience</span></div></div><div className="flex flex-wrap items-center gap-3"><select aria-label="Leave year" value={year} onChange={(event) => setYear(Number(event.target.value))} className="min-h-12 rounded-xl border border-white/15 bg-white/10 px-4 font-bold text-white shadow-lg backdrop-blur"><option className="text-slate-900" value={currentYear - 1}>{currentYear - 1}</option><option className="text-slate-900" value={currentYear}>{currentYear}</option><option className="text-slate-900" value={currentYear + 1}>{currentYear + 1}</option></select><button type="button" onClick={() => setRequestOpen(true)} className="min-h-12 rounded-xl bg-white px-5 text-sm font-bold text-[#0F172A] shadow-xl transition hover:-translate-y-0.5 hover:bg-cyan-50">+ Request Leave</button></div></div>
      </header>

      <div className="relative z-10 -mt-4 mx-3 flex gap-2 overflow-x-auto rounded-2xl border border-white/80 bg-white/95 p-2 shadow-[0_18px_45px_-28px_rgba(15,23,42,.7)] backdrop-blur sm:mx-6">{tabs.map(([value, label, count, icon]) => <button key={value} type="button" onClick={() => setTab(value)} className={`group flex items-center gap-2 whitespace-nowrap rounded-xl px-4 py-2.5 text-sm font-bold transition duration-200 ${tab === value ? "bg-gradient-to-r from-[#153E90] to-blue-700 text-white shadow-lg shadow-blue-900/20" : "text-slate-500 hover:bg-blue-50 hover:text-[#153E90]"}`}><TimeOffIcon name={icon} className="h-4 w-4" />{label}{count !== null ? <span className={`rounded-full px-2 py-0.5 text-[10px] ${tab === value ? "bg-white/15" : "bg-slate-100 group-hover:bg-white"}`}>{count}</span> : null}</button>)}</div>

      <div className="mt-7">
        {loading ? <LoadingState /> : error || !data ? <div role="alert" className="rounded-2xl border border-red-200 bg-red-50 p-6 text-red-800"><p className="font-bold">Unable to load Time Off</p><p className="mt-1 text-sm">{error}</p><KairoButton type="button" className="mt-4" onClick={refresh}>Try Again</KairoButton></div> : <>{tab === "overview" ? <TimeOffDashboard dashboard={data.dashboard} requests={data.requests} holidays={data.holidays} notifications={data.notifications} onChanged={refresh} onRequestLeave={() => setRequestOpen(true)} /> : null}{tab === "requests" ? <MyLeaveRequests requests={data.requests} leaveTypes={data.leaveTypes} onChanged={refresh} /> : null}{tab === "calendar" ? <LeaveCalendar leave={data.calendar} holidays={data.holidays} /> : null}{tab === "approvals" && canApprove ? <ApprovalCentre requests={data.approvalQueue} managedRequests={data.managedRequests} calendar={data.calendar} balances={data.managerBalances} recent={data.recentTeamRequests} isAdmin={isAdmin} onChanged={refresh} /> : null}{tab === "admin" && canAdminister && adminData ? <TimeOffAdmin data={adminData} leaveTypes={data.leaveTypes} holidays={data.holidays} onChanged={refresh} /> : null}<RequestLeaveDialog open={requestOpen} employeeId={data.profile.employee_id} employeeGender={data.profile.gender} leaveTypes={data.leaveTypes} isAdmin={isAdmin} onClose={() => setRequestOpen(false)} onSubmitted={refresh} /></>}
      </div>
    </div>
  </main>;
}
