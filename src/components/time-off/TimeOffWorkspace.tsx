  ...
  return <main className="min-h-screen bg-[#f8fafc] px-5 py-7 sm:px-8">
    <div className="mx-auto max-w-[1500px]">
      <header className="relative overflow-hidden rounded-[2rem] bg-gradient-to-br from-[#081226] via-[#102656] to-[#153E90] px-7 py-8 text-white shadow-[0_28px_75px_-35px_rgba(15,23,42,.85)] sm:px-10 lg:py-10">
        ...
        <div className="relative flex flex-wrap items-center justify-between gap-6"><div><p className="text-xs font-bold uppercase tracking-[0.22em] text-cyan-200">People & wellbeing</p><h1 className="mt-2 text-4xl font-bold tracking-tight lg:text-5xl">Time Off</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-slate-300">Plan leave, protect team capacity, and keep every policy decision clear and auditable.</p><span className="mt-4 inline-flex rounded-full border border-white/10 bg-white/10 px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-blue-100 backdrop-blur">{role || "employee"} experience</span></div><div className="flex flex-wrap items-center gap-3"><select aria-label="Leave year" value={year} onChange={(event) => setYear(Number(event.target.value))} className="min-h-12 rounded-xl border border-white/15 bg-white/10 px-4 font-bold text-white shadow-lg backdrop-blur"><option className="text-slate-900" value={currentYear - 1}>{currentYear - 1}</option><option className="text-slate-900" value={currentYear}>{currentYear}</option><option className="text-slate-900" value={currentYear + 1}>{currentYear + 1}</option></select><button type="button" onClick={() => setRequestOpen(true)} className="min-h-12 rounded-xl bg-white px-5 text-sm font-bold text-[#0F172A] shadow-xl transition hover:-translate-y-0.5 hover:bg-cyan-50">+ Request Leave</button></div></div>
      </header>
