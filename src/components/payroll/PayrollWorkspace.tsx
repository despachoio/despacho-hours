...
function Settings({value,bankDetails,financeAccess,onSave}:{value:PayrollSettings;bankDetails?:CompanyPayrollBankDetails;financeAccess:boolean;onSave:(p:Record<string,unknown>)=>Promise<boolean>}) {
  ...
  <Card className="p-7"><p className="text-xs font-bold uppercase tracking-[.18em] text-[#153E90]">Secure employee delivery</p><h2 className="mt-2 text-xl font-bold">Payslip Distribution</h2><p className="mt-2 text-slate-500">Control email delivery and password protection applied immediately after payroll submission.</p><fieldset className="mt-6 grid gap-3 lg:grid-cols-3"><legend className="mb-3 text-sm font-bold">Distribution Method</legend>{[["notify_only","Notify employee by email only"],["protected_pdf_only","Send password-protected PDF only"],["notify_and_attach","Notify employee + Attach password-protected PDF"]].map(([key,label])=><label key={key} className={`flex cursor-pointer items-center gap-3 rounded-2xl border p-4 text-sm font-bold ${form.distributionMethod === key ? "border-[#153E90] bg-blue-50 text-[#153E90]" : "border-slate-200"}`}><input type="radio" checked={form.distributionMethod === key} onChange={()=>setForm({...form,distributionMethod:key as PayrollSettings["payslip_distribution_method"]})}/>{label}</label>)}</fieldset><div className="mt-5 grid gap-4 md:grid-cols-2"><label className="flex items-center justify-between rounded-2xl border border-slate-200 p-4 text-sm font-bold">Password Protection<input type="checkbox" checked={form.passwordProtection} onChange={e=>setForm({...form,passwordProtection:e.target.checked})} className="h-5 w-5"/></label><label className="text-sm font-bold">Password Rule<select disabled={!form.passwordProtection} value={form.passwordRule} onChange={e=>setForm({...form,passwordRule:e.target.value as PayrollSettings["payslip_password_rule"]})} className={fieldClass}><option value="employee_code_dob">Employee Code + DOB (DDMMYYYY)</option><option value="employee_code">Employee Code Only</option><option value="dob">DOB (DDMMYYYY)</option></select></label></div><label className="mt-5 block text-sm font-bold">Email Subject<input value={form.emailSubject} onChange={e=>setForm({...form,emailSubject:e.target.value})} className={fieldClass}/></label><label className="mt-5 block text-sm font-bold">Email Template<textarea value={form.emailTemplate} onChange={e=>setForm({...form,emailTemplate:e.target.value})} className={`${fieldClass} min-h-56 leading-6`}/><span className="mt-2 block text-xs font-medium text-slate-500">Variables: {"{{EmployeeName}} {{PayrollMonth}} {{CompanyName}} {{PortalUrl}} {{PasswordRuleDescription}}"}</span></label><KairoButton type="button" className="mt-6" onClick={()=>void saveSettings()}>Save Payroll Settings</KairoButton></Card>
