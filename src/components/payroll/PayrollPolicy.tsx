...
function PolicyIcon({ name }: { name: "shield" | "calendar" | "wallet" | "calculator" | "flow" }) {
...
const sectionIcons: Record<string, Parameters<typeof PolicyIcon>[0]["name"]> = { governance: "shield", period: "calendar", structure: "wallet", calculation: "calculator", processing: "flow" };

export default function PayrollPolicy() {
  const [openSections, setOpenSections] = useState(() => new Set(["governance"]));
  ...
  return <div className="space-y-6">
    <section className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-[#0F172A] via-[#172554] to-[#153E90] px-7 py-8 text-white shadow-xl"><div className="absolute -right-10 -top-20 h-64 w-64 rounded-full bg-cyan-400/15 blur-3xl"/><div className="relative max-w-3xl"><p className="text-xs font-bold uppercase tracking-[.22em] text-cyan-200">Payroll knowledge centre</p><h2 className="mt-3 text-3xl font-bold tracking-tight">Payroll Policy</h2><p className="mt-3 text-sm leading-7 text-blue-100">A clear guide to payroll periods, salary calculations, statutory deductions, processing controls, and employee access.</p><div className="mt-5 flex flex-wrap gap-2"><span className="rounded-full border border-white/15 bg-white/10 px-3 py-1 text-xs font-semibold">Visible to everyone</span><span className="rounded-full border border-white/15 bg-white/10 px-3 py-1 text-xs font-semibold">April–March financial year</span><span className="rounded-full border border-white/15 bg-white/10 px-3 py-1 text-xs font-semibold">Last reviewed August 2026</span></div></div></section>
